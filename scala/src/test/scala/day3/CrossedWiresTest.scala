...
class CrossedWiresTest extends AnyFunSuite with Matchers {
  ...
  test("Parse wire") {
    CrossedWires.parseWire("R2,U2") shouldBe Wire(Vector(R(2), U(2)))
    CrossedWires.parseWire("L5,D10") shouldBe Wire(Vector(L(5), D(10)))
  }

  test("Horizontal points") {
    CrossedWires.pointsHorizontal((0, 0), 2) shouldBe Vector((1, 0), (2, 0))
    CrossedWires.pointsHorizontal((0, 0), -2) shouldBe Vector((-1, 0), (-2, 0))
    CrossedWires.pointsHorizontal((5, 2), -3) shouldBe Vector(
      (4, 2),
      (3, 2),
      (2, 2)
    )
  ...
  test("Vertical points") {
    val p1 = (0, 0)
    CrossedWires.pointsVertical(p1, 2) shouldBe Vector((0, 1), (0, 2))
    CrossedWires.pointsVertical(p1, -2) shouldBe Vector((0, -1), (0, -2))
  }

  test("PathFromOrigin") {
    Wire(Vector(D(2), R(2), U(3), L(1))).pathFromOrigin shouldBe Vector(
      (0, 0),
      (0, -1),
      (0, -2),
      (1, -2),
  ...
  test("Intersections") {
    ...
    CrossedWires.intersections(
      Wire(Vector(R(8), U(5), L(5), D(3))),
      Wire(Vector(U(7), R(6), D(4), L(4)))
